...
# Updated dictionary with phrases and individual words
translation_dict = {
    # Individual words
    ...
    "break a leg": {"word": "शुभकामनाएँ", "pos": "PHRASE"},
...
def apply_hindi_grammar(translations, original_sentence):
    ...
    for word, pos in translations:
        if pos.startswith("NN") or pos == "PRP" or pos == "PRP$":
            subject.append(word)
        elif pos == "IN" or pos == "JJ":
            objects.append(word)
        elif pos.startswith("VB") or pos == "VBG":
            verbs.append(word)
        elif pos == "VBP_FUTURE" or pos == "VBD" or pos == "VBP":
            auxiliaries.append(word)

    # Construct Hindi sentence: Subject → Object → Verb → Auxiliary
    ordered_sentence = subject + objects + verbs + auxiliaries
